...
uint32_t lcd_init(lcd_conf_t* lcd_conf, spi_device_handle_t *spi_wr_dev, lcd_dc_t *dc, int dma_chan)
{
    ...
    if (lcd_conf->init_spi_bus) {
        ...
        esp_err_t ret = spi_bus_initialize(lcd_conf->spi_host, &buscfg, SPI_DMA_CH_AUTO);
    ...
    esp_err_t ret = spi_bus_add_device(lcd_conf->spi_host, &devcfg, &rd_id_handle);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "spi_bus_add_device(read) failed: %s", esp_err_to_name(ret));
        return 0;
    }
    ...
    ret = spi_bus_add_device(lcd_conf->spi_host, &devcfg, spi_wr_dev);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "spi_bus_add_device(write) failed: %s", esp_err_to_name(ret));
        return 0;
    }
    ...
    if (lcd_conf->pin_num_bckl < GPIO_NUM_MAX) {
        gpio_reset_pin(lcd_conf->pin_num_bckl);
